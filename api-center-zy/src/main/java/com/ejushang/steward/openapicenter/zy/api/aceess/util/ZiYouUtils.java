...
import com.ejushang.steward.openapicenter.zy.api.aceess.parser.JsonReader;
import com.ejushang.steward.openapicenter.zy.api.aceess.parser.JsonValidatingReader;
 ...
 * Time: 16:14
 */
public abstract class ZiYouUtils {
    ...
    public static String signTopRequest(RequestParametersHolder requestHolder, String secret) throws IOException {
        ...
        for (String key : keys) {
            ...
            if (com.taobao.api.internal.util.StringUtils.areNotEmpty(key, value)) {
    ...
    private static byte[] encryptHMAC(String data, String secret) throws IOException {
        byte[] bytes = null;
        try {
            SecretKey secretKey = new SecretKeySpec(secret.getBytes(com.taobao.api.Constants.CHARSET_UTF8), "HmacMD5");
            Mac mac = Mac.getInstance(secretKey.getAlgorithm());
            mac.init(secretKey);
            bytes = mac.doFinal(data.getBytes(com.taobao.api.Constants.CHARSET_UTF8));
    ...
    private static String getStringFromException(Throwable e) {
        ...
        try {
            result = bos.toString(com.taobao.api.Constants.CHARSET_UTF8);
    ...
    private static byte[] encryptMD5(String data) throws IOException {
        ...
        try {
            ...
            bytes = md.digest(data.getBytes(com.taobao.api.Constants.CHARSET_UTF8));
        } catch (GeneralSecurityException gse) {
            String msg = getStringFromException(gse);
            throw new IOException(msg);
        }
   ...
   /* *//**
    ...
    public static TaobaoContext getTopContext(String authCode) throws IOException {
        ...
        String rsp = com.taobao.api.internal.util.WebUtils.doGet(url, null, com.taobao.api.Constants.CHARSET_UTF8);
        if (com.taobao.api.internal.util.StringUtils.isEmpty(rsp)) {
            return null;
        }

        TaobaoContext context = new TaobaoContext();

        Set<Map.Entry<String, String>> paramSet = com.taobao.api.internal.util.WebUtils.splitUrlQuery(rsp).entrySet();
    ...
    public static Map<String, String> decodeTopParams(String topParams) throws IOException {
        return decodeTopParams(topParams, com.taobao.api.Constants.CHARSET_GBK);
    }
    ...
    public static Map<String, String> decodeTopParams(String topParams,String charset) throws IOException {
        if (com.taobao.api.internal.util.StringUtils.isEmpty(topParams)) {
            return null;
        }

        byte[] buffer = Base64.decode(com.taobao.api.internal.util.WebUtils.decode(topParams).getBytes());
    ...
    public static String getProductWebSessionUrl(String appKey) {
        StringBuilder url = new StringBuilder(com.taobao.api.Constants.PRODUCT_CONTAINER_URL);
     ...
     * @return API响应对象
     */
    public static <T extends ZiYouResponse> T parseResponse(String json, Class<T> clazz) throws ApiException {
